...
const RealisticOceanBackground = () => {
  ...
  useEffect(() => {
    ...
    // Modified "Buffer A" shader code
    // iChannelX dependencies are replaced with simple_noise or flat colors
    const fragmentShaderSource = `#version 300 es
      ...
      uniform float iTime;
      uniform vec4 iMouse;
      // uniform sampler2D iChannel0; // Removed
      // uniform sampler2D iChannel1; // Removed
      // uniform sampler2D iChannel2; // Removed
      // uniform sampler2D iChannel3; // Removed
      out vec4 fragColor;
      ...
      vec3 simple_noise3(vec2 p) {
          return vec3(simple_noise(p), simple_noise(p + vec2(10.0, 10.0)), simple_noise(p + vec2(20.0, 20.0)));
      }
      ...
      float noise( in vec2 p ) { // This is the user's original noise, not to be confused with simple_noise
          vec2 i = floor( p );
          vec2 f = fract( p );	
      ...
      float fbmClouds(in vec2 uv)
      {
          ...
          float f = simple_noise(uv); // Replacement for iChannel3
          ...
          f += simple_noise(uv*2.0) * 0.5*0.5; // Replacement
          // f += texture(iChannel3, uv*4.0).r * 0.5*0.5*0.5;
          f += simple_noise(uv*4.0) * 0.5*0.5*0.5; // Replacement
          // f += texture(iChannel3, uv*32.0).r * 0.5*0.5*0.5*0.5*0.5;
          f += simple_noise(uv*32.0) * 0.5*0.5*0.5*0.5*0.5; // Replacement
          // f += texture(iChannel3, uv*8.0).r * 0.5*0.5*0.5*0.5;
          f += simple_noise(uv*8.0) * 0.5*0.5*0.5*0.5; // Replacement
          // f += texture(iChannel3, uv*64.0).r * 0.5*0.5*0.5*0.5*0.5;
          f += simple_noise(uv*64.0) * 0.5*0.5*0.5*0.5*0.5; // Replacement
          return f;
      }

      // water
      float fbm(in vec2 uv, float lod) // lod parameter is not used with simple_noise
      {	
          // float f =  textureLod(iChannel2, uv, lod).r * 0.5;
          float f =  simple_noise(uv) * 0.5; // Replacement for iChannel2
      ...
      float wavesLo(const in vec3 rp, float t, float A, float lod)
      {
          ...
          float w00 = noise(uv * 1.2 * vec2(.2,  2.2) + vec2(0.20 * t, t * 1.5)); // User's noise
          float w01 = fbm(uv*.05+vec2(0.0, iTime*.01), lod); // Uses our modified fbm
          float w02 = fbm(uv*.1 +vec2(iTime*.002, iTime*.014), lod); // Uses our modified fbm
      ...
      {
          vec2 uv = rp.xz;
          vec2 nuv2 = uv+0.05*vec2(noise(uv*5.+t*.03), noise(uv*2.+t*.04)); // User's noise
      ...
      vec3 texGrad(in vec2 uv) // For spec bump
      {
          vec2 offset = vec2(0.01, 0.0);
          uv *= .5;
          // float h0 = texture(iChannel0, uv).r;
          float h0 = simple_noise(uv); // Replacement for iChannel0
          // float h1 = h0-texture(iChannel0, uv+offset.xy).r;
          float h1 = h0-simple_noise(uv+offset.xy); // Replacement
          // float h2 = h0-texture(iChannel0, uv+offset.yx).r;
          float h2 = h0-simple_noise(uv+offset.yx); // Replacement
      ...
      void trace(in vec3 rp, in vec3 rd, inout vec4 color)
      {
          ...
          if (t < 0.0) 
          {
              // If ray doesn't hit the plane, keep current sky color and return
              // color.a is used for depth later, set it to a large value for sky
              color.a = 10000.0; // Large depth for sky
              return;
          }
          ...
          for (int i = 0; i < 10; ++i)
          {
              rp += dist * rd; // This line was potentially problematic if dist was large from previous loop.
                               // Let's ensure dist is from map(rp) before adding.
              dist = map(rp);
              if (abs(dist) < 0.0001) break;
              // rp += dist * rd; // Moved map(rp) before this
          }
          
          if(hit)
          {        
              vec3 g = grad(rp, 0.002 * (1.0+log2(length(ro-rp) * 100.)));
              color = vec4(.05, 0.22, .4, 0.0); // Base color
              float d_diffuse = max(0.0, dot(g, lightDir));
              color.rgb *= mix(vec3(1.0), vec3(d_diffuse), .4); // Diffuse
              
              vec2 heights = sub(rp, 0.3);
              float sst = length(max(vec2(0.00001), heights));
              float sunD = -1.*(dot(sun, g));
              color.rgb = mix(color.rgb, vec3(.2, 0.5, 0.72)*.5, smoothstep(0., .4, sunD)); // Top
              
              float ssb = length(min(vec2(0.0), heights));
              color.rgb = mix(color.rgb, vec3(.05, 0.22, .4)*.5, smoothstep(0.0, 0.4, ssb)); // Bottom
              
              // Foam
              vec3 pw = vec3(1.+2.3/(sst*1.+.001))*2.2;
              // vec3 texcol1 = texture(iChannel0, rp.xz*.5+vec2(.0, .07*iTime)).rgb;
              vec3 texcol1 = simple_noise3(rp.xz*.5+vec2(.0, .07*iTime)); // Replacement
              
              vec3 pw2 = vec3(1.+.7/(sst*1.2+.01));
              // vec3 texcol2 = texture(iChannel0, rp.xz*.5+vec2(.0, .07*iTime)).rgb*1.;
              vec3 texcol2 = simple_noise3(rp.xz*.5+vec2(.0, .07*iTime)); // Replacement
              ...
              color.rgb += .2*pow(F, 6.)*simple_noise3(reflect(rd, g).xy); // Replacement for iChannel1
              ...
              // If no hit, use the sky color calculated at the beginning
              // color.a should already be set if t < 0
              if (t >= 0.0) color.a = 5000.0; // Set a far depth if plane was hit but not water surface
          }
          color.a = length(ro-rp); // This is the depth value for the Image pass.
                                   // For single pass, we can ignore it or use it for fog.
                                   // For now, let's ensure fragColor.a is 1.0 for visible output.
      }

      mat3 lookat(vec3 from, vec3 to)
      {
          vec3 f = normalize(to - from);
          vec3 _tmpr = normalize(cross(f, vec3(0.0, .999, 0.0))); // Ensure up vector is not parallel
          if (length(_tmpr) < 0.001) _tmpr = normalize(cross(f, vec3(0.001, .999, 0.0))); // Handle edge case
      ...
      {
          vec4 computedColor = vec4(0.0); // Initialize to black
          vec2 uv = (fragCoord.xy-iResolution.xy*.5) / iResolution.x;
          vec2 im = 4.0 * ((iMouse.xy / iResolution.xy) - vec2(0.5));
          
          if (iMouse.z <= 0.0 && iMouse.w <= 0.0) // Check if mouse is not pressed
          {
              im = vec2(.1, .2); // Default view if mouse not used
          }
          
          vec3 rd = normalize(vec3(uv, (1.0-.2*length(uv))));
          vec3 rp = vec3(0.0, .3, -1.0);
          vec3 _rp_clouds = rp; // Separate rp for clouds, as original shader did
          
          rp = roty(im.x) * rp; // Apply mouse rotation
          ...
          // Clouds (simplified, as _rp was used for clouds before mouse rotation in original)
          float t_clouds = -(6.5+dot(_rp_clouds, -up)) / dot(-up, rd); // Use rd after lookat for consistency
          if (t_clouds>0.)
          {
              ...
              // computedColor.a = t_clouds; // Not strictly needed for single pass color
          }
          computedColor.rgb = clamp(computedColor.rgb, 0.0, 1.0);
          finalColor = vec4(computedColor.rgb, 1.0); // Ensure alpha is 1.0 for direct screen rendering
      }
    ...
    function createProgram(glCtx: WebGL2RenderingContext, vertexShader: WebGLShader, fragmentShader: WebGLShader) {
      ...
      if (!glCtx.getProgramParameter(program, gl.VALIDATE_STATUS)) {
        ...
        // return null; // Can still try to run if validation fails
      }
      return program
    }
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, positionBuffer) // Bind again before vertexAttribPointer
    gl.vertexAttribPointer(positionAttributeLocation, 2, gl.FLOAT, false, 0, 0)

    let mouseX = 0, mouseY = 0, mouseClickX = 0, mouseClickY = 0;
    const handleMouseMove = (e: MouseEvent) => {
      mouseX = e.clientX 
      mouseY = canvas.height - e.clientY // Standard GL Y-coordinate (bottom-up)
    };
    const handleMouseDown = (e: MouseEvent) => {
     ...
     const handleMouseUp = (e: MouseEvent) => {
        // Shadertoy uses iMouse.zw for mouse click state, often -ve when not clicked after a click
    ...
    function resizeCanvas() {
      ...
      // Set mouse to center initially if not moved
    ...
    function renderLoop() {
      if (!gl || !program) { // Ensure program is valid
      ...
      // Reset mouse click state for next frame if it was positive (clicked)
      // This emulates Shadertoy's iMouse.zw behavior somewhat
