    ...
    const fragmentShaderSource = `#version 300 es
      ...
      void trace(in vec3 rp, in vec3 rd, inout vec4 color) {
          ...
          if(hit) {        
              ...
              vec3 texcol1 = vec3(proc_fbm_noise_for_channel0(rp.xz*.5+vec2(.0, .07*iTime))); // Replacement
              
              vec3 pw2 = vec3(1.+.7/(sst*1.2+.01));
              vec3 texcol2 = vec3(proc_fbm_noise_for_channel0(rp.xz*.5+vec2(.0, .07*iTime))); // Replacement
              ...
              color.rgb += .2*pow(F, 6.)*proc_sky_for_channel1(reflect(rd, g), 0.0); // Replacement
